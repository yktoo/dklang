///*********************************************************************************************************************
///  $Id: DKL_Expt.pas 2013-12-06 00:00:00Z bjm $
///---------------------------------------------------------------------------------------------------------------------
...
resourcestring
  ...
  SDKLExptMenuItem_EditConstants     = 'DKLang - Edit pro&ject constants...';
  SDKLExptMenuItem_UpdateLangSource  = 'DKLang - Update project lan&guage source';

implementation //=======================================================================================================

uses
  VCL.Menus, VCL.Dialogs, VCL.Graphics,
  System.SysUtils, System.Types, System.TypInfo, DesignIntf, Winapi.Windows,
  ...
  begin
    ...
    if Result=nil then raise EDKLangError.Create(SDKLExptErr_CannotFindProject);
  end;
...
type
  ...
  begin
    ...
    if not Supports(BorlandIDEServices, INTAServices,       FNTAServices) then raise EDKLangError.Create(SDKLExptErr_CannotObtainNTAIntf);
    if not Supports(BorlandIDEServices, IOTAServices,       FOTAServices) then raise EDKLangError.Create(SDKLExptErr_CannotObtainOTAIntf);
    if not Supports(BorlandIDEServices, IOTAModuleServices, FModServices) then raise EDKLangError.Create(SDKLExptErr_CannotObtainModSvcIntf);
    ...
    if mi=nil then raise EDKLangError.Create(SDKLExptErr_CannotFindProjectMenu);
  ...
  var
    ...
    function GetProjectDprFileName: UnicodeString;
    begin
      Result := ChangeFileExt(GetActualProject.FileName, '.dpr')
    end;
        ...
        if Result then
          ...
          if bErase then begin
           ...
           // Else save the constants back to the resources
            ...
            if Consts.AutoSaveLangSource and not UpdateProjectLangSource(Consts) then raise EDKLangError.Create(SDKLExptErr_CannotSaveLangSource);
            // maybe update the project source to include {$R *.dkl_const.res}
            if FileExists(GetProjectDprFileName) then begin
              projectSource := TStringList.Create;
              try
                projectSource.LoadFromFile(GetProjectDprFileName);
                s := LowerCase(projectSource.Text);
                if (Pos(LowerCase(SDKLExpt_ConstantResFileEnding),s)=0) then begin
                  // not already there, so insert it
                  // insert right after {$R *.res}
                  s := '{$r *.res}';
                  for i:=0 to projectSource.Count-1 do begin
                    if Pos('{$r *.res}',LowerCase(projectSource[i]))>0 then begin
                      projectSource.Insert(i+1,'{$R *' + SDKLExpt_ConstantResFileEnding + '}');//{$R *.dkl_const.res}
                      projectSource.SaveToFile(GetProjectDprFileName);
                      break;
                    end;
                  end;
                end;
  ...
  begin
    ...
    try
      bitmap.LoadFromResourceName(HInstance,'DKLangMenuIcon');
      Result.ImageIndex := Menu.GetParentMenu.Images.AddMasked(bitmap, bitmap.TransparentColor);
    finally
      bitmap.Free;
    end;
  ...
  begin
    ...
    case Index of
      ...
      0: if not UpdateProjectLangSource(LC) then raise EDKLangError.Create(SDKLExptErr_CannotSaveLangSource);
  ...
  procedure DKLang_AddSplashText;
  var
    bmp: VCL.Graphics.TBitmap;
  begin
    bmp := VCL.Graphics.TBitmap.Create;
    bmp.LoadFromResourceName(HInstance, 'TDKLanguageController');
    SplashScreenServices.AddPluginBitmap('DKLang Localization Package © 2013 by Yktoo',bmp.Handle,false,'Open Source!','v5');
    bmp.Free;
  end;


initialization
   DKLang_AddSplashText;
